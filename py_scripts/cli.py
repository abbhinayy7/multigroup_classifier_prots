...
def plot_ranked_samples(pred_tbl, output_path, threshold=None, title="Ranked Predicted Samples"):
    """Plot ranked prediction scores with color coding for labels and detailed description."""
    fig, ax = plt.subplots(figsize=(13, 8))
    
    if 'ann' in pred_tbl.columns:
        # Get unique labels and map to colors
        unique_labels = sorted(pred_tbl['ann'].unique())
        colors = {'0': '#1f77b4', '1': '#d62728'}  # Blue and Red
        
        for idx, label in enumerate(unique_labels):
            mask = pred_tbl['ann'] == label
            color = colors.get(str(idx), '#1f77b4')
            label_str = f'{label} (n={mask.sum()})'
            ax.scatter(pred_tbl[mask]['rank'], pred_tbl[mask]['preds'], 
                      s=120, alpha=0.75, label=label_str, color=color, edgecolors='black', linewidth=0.5)
    else:
    ...
    if threshold is not None and not np.isnan(threshold):
        ...
        ax.fill_between([0, len(pred_tbl)+1], threshold, 1.05, alpha=0.12, color='green', label='Predicted Positive Region')
        ax.fill_between([0, len(pred_tbl)+1], -0.05, threshold, alpha=0.12, color='red', label='Predicted Negative Region')
    ...
    ax.text(0.5, -0.12, desc_text, transform=ax.transAxes, ha='center', fontsize=10, style='italic', weight='bold')
...
def train_command(args):
    try:
        ...
        def xgb_cv_auc(eta, max_depth, subsample, colsample_bytree,
            ...
            cv = xgb.cv(params, dtrain, num_boost_round=1500, nfold=5,
                        early_stopping_rounds=100, verbose_eval=False, stratified=True, seed=args.seed)
            return cv['test-auc-mean'].max()

        # Optimized hyperparameter bounds for better performance
        pbounds = {
            'eta': (0.001, 0.5),
            'max_depth': (2, 15),
            'subsample': (0.4, 1.0),
...
def evaluate_command(args):
    try:
        ...
        try:
            ...
            # ROC curve with enhanced visualization
            ...
            # Mark the operating point (best threshold)
            if not np.isnan(best_thr):
                # Find the closest point to best_thr on the ROC curve
                thresholds = roc_curve(y, preds)[2]
                op_idx = np.argmin(np.abs(thresholds - best_thr))
                ax.plot(fpr[op_idx], tpr[op_idx], 'o', markersize=14, color='red', 
                       label=f'Operating Point (Threshold = {best_thr:.4f})', zorder=5, markeredgecolor='darkred', markeredgewidth=2)
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(outdir, f'roc_curve_{ts}.png'), dpi=150, bbox_inches='tight')
            plt.close()
            logger.info('ROC curve written as PNG with performance metrics.')
        ...
        try:
            ...
            if has_labels and not np.isnan(base_thr):
                ...
                if n_kept > 0:
                    ...
                    sens = recall_score(y_true, pred_class, zero_division=0)
                    ...
                    # ROC curve for ad-hoc application
                    if len(np.unique(y_true)) == 2 and n_kept >= 2:
                        ...
                        op_idx = np.argmin(np.abs(thresholds - base_thr))
                        ax.plot(fpr[op_idx], tpr[op_idx], 'o', markersize=14, color='red',
                               label=f'Operating Point (Threshold = {base_thr:.4f})', zorder=5, markeredgecolor='darkred', markeredgewidth=2)
                        
                        ax.set_xlim(-0.02, 1.02)
                        ax.set_ylim(-0.02, 1.02)
                        ax.set_xlabel('False Positive Rate (1 - Specificity)', fontsize=13, fontweight='bold')
                        ax.set_ylabel('True Positive Rate (Sensitivity)', fontsize=13, fontweight='bold')
                        ax.set_title('ROC Curve - Ad-hoc Application (Classified Samples)', fontsize=15, fontweight='bold', pad=20)
                        ax.legend(loc='lower right', fontsize=12, framealpha=0.95, edgecolor='black', fancybox=True)
                        ax.grid(True, alpha=0.3, linestyle='--', linewidth=0.8)
                        
                        # Add metrics text box
                        metrics_text = f'Ad-hoc Performance\nAccuracy: {acc:.2%}\nSensitivity: {sens:.2%}\nSpecificity: {spec:.2%}\nPrecision: {prec:.2%}'
                        ax.text(0.98, 0.02, metrics_text, transform=ax.transAxes, fontsize=11, weight='bold',
                               verticalalignment='bottom', horizontalalignment='right',
                               bbox=dict(boxstyle='round', facecolor='#fff9e6', alpha=0.95, edgecolor='black', linewidth=1.5))
                        
                        plt.tight_layout()
                        plt.savefig(os.path.join(outdir, f'roc_curve_{ts}_adhoc.png'), dpi=150, bbox_inches='tight')
                        plt.close()
                        logger.info('Ad-hoc ROC curve written as PNG with performance metrics.')
